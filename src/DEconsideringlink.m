...
%i=1;
    for j=1:1:No_Users
       ...
       % [xfac,ind]=sort(XF,'descend');
        if(j==4)
           [xfac,ind]=sort(XF,'descend');
           Erem(ind(1,i),i+1)=Erem(ind(1,i),i)-(1000*(50e-9+(100e-12*d_mean(1,i+1)*d_mean(1,i+1))));
           
           for z=1:1:No_Users
             if(z==ind(1,i))
                 continue
             else
              Erem(z,i+1)=Erem(z,i)-(1000*((50*10e-9)+(100*10e-12*d(1,i+1,z)*d(1,i+1,z))));;
             end   
           end
        end 
    end
